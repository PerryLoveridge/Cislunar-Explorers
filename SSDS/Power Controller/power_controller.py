...
class Power(object):
    ...
    #		  AssertionError if cmd or values are not correct type
    ...
    # raises: AssertionError if bytes is not an int
    def read(self, bytes):
    	assert type(bytes) == int
    ...
    # raises: AssertionError if incorrect type
    #		  AssertionError if channel or value are out of range
    # 		  AssertionError if delay is not a number
    def set_single_output(self, channel, value, delay):
    	assert type(channel) == int and type(value) == int and type(delay) == int
    ...
    def set_pv_volt(self, volt1, volt2, volt3):
    	assert type(volt1) == int and type(volt2) == int and type(volt3) == int
